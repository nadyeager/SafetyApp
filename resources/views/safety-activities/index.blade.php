            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Frequency</th>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Notes
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">File</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($safetyActivities as $i => $act)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-700">{{ \Carbon\Carbon::parse($act->date)->format('Y-m-d') }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $act->type }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $act->frequency }}</td>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-700">{{ \Illuminate\Support\Str::limit($act->notes, 80) }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-700">
                                @if($act->file)
                                    <a href="{{ asset('storage/' . $act->file) }}" target="_blank"
                                ...
                                <!-- <td class="px-4 py-3 text-sm">
                                    <div class="flex space-x-3">
                                        <a href="{{ route('safety-activities.edit', $act) }}" class="text-blue-600 hover:text-blue-800 font-medium">Edit</a>
                                        <form action="{{ route('safety-activities.destroy', $act) }}" method="POST" onsubmit="return confirm('Hapus activity ini?');">
                                            @csrf
                                            @method('DELETE')
