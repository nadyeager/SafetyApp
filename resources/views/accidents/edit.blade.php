    ...
    <form action="{{ route('accidents.update', $accident->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div>
            ...
            <select name="category" id="category" class="mt-1 block w-full border-gray-300 rounded">
                ...
                <option value="work accident" {{ old('category', $accident->category) == 'work accident' ? 'selected' : '' }}>
                    Work Accident</option>
        ...
        <div>
            ...
            <select name="type" id="type" class="mt-1 block w-full border-gray-300 rounded">
                ...
                <option value="Major injury" {{ old('type', $accident->type) == 'Major injury' ? 'selected' : '' }}>Major
                    injury</option>
                <option value="Minor injury" {{ old('type', $accident->type) == 'Minor injury' ? 'selected' : '' }}>Minor
                    injury</option>
                <option value="Property damage" {{ old('type', $accident->type) == 'Property damage' ? 'selected' : '' }}>
                    Property damage</option>
                <option value="Non Work Accident" {{ old('type', $accident->type) == 'Non Work Accident' ? 'selected' : '' }}>
                    Non Work Accident</option>
        ...
        <div>
            ...
            @if ($accident->file)
                <img src="{{ asset('storage/' . $accident->file) }}" alt="{{ $accident->file }}" width="100" class="mb-2">
            @endif
